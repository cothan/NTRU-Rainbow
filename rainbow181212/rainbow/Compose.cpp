			...
			unsigned char *pk, unsigned long *pklen)
{
	...
	for (k0 = 0; k0 < un - 1; k0++)
	{
		...
		for (; k < Slist[k0 + 1] - v1; k++)
		{
			...
			for (i = 0; i < vinegar + oil; i++)
				c[k][i] = sk[ij++] = randombyte() % deg_pow;
			if (oil == 1)
			{
				i--;
				while (c[k][i] == 0)
	...
	*sklen = ij;
	...
	for (k0 = 0; k0 < un - 1; k0++)
	{
		...
		for (; k < Slist[k0 + 1] - v1; k++)
		{
			...
			for (i = 0; i < vinegar; i++)
			{
				...
				for (j = 0; j < oil; j++)
					C[i * ovn1 + j + vinegar] = a[k][i * oil + j];
			}
			for (j = 0; j < vinegar + oil; j++)
				C[ovn * ovn1 + j] = c[k][j];
	...
	for (k = 0; k < nv1; k++)
	{
		...
		for (i = 0; i < ovn1 * ovn1; i++)
			D[k][i] = 0; // may not be needed, just in case
		for (j = 0; j < nv1; j++)
		{
			for (i = 0; i < ovn1 * ovn1; i++)
				D[k][i] += S[k * nv1 + j] * D0[j][i];
		}
	...
	for (k = 0; k < nv1; k++)
	{
		...
		{
			for (j = i; j < ovn1; j++)
				pk[ij++] = D[k][i * ovn1 + j].GFpowtochar();
		}
	}
...
// matrix A is set up to hold resulting linear equations
{
	...
	do
	{
		...
		repeat = 0;
		for (j = 0; j < v1; j++)
			x[j] = randombyte() % deg_pow;

		for (k0 = 0; k0 < un - 1; k0++)
		{

			//if (k0 == 0) vinegar = Slist[k0]-1;
			//else vinegar = Slist[k0];
			vinegar = Slist[k0];
			oil = Slist[k0 + 1] - vinegar;
			if (oil == 1)
			{ // special case   mixed terms set to zero
				// no difficulty in solving equation
				// constant term caused by vinegar, subtract from y1[k]
				k1 = vinegar - v1;			   //reparameterization changes not added here
			...
			{ // general case
				for (k = 0; k < oil; k++)
				{
					//if (k0 == 0) k1 = k + vinegar - v1 - 1;
					//else k1 = k + vinegar - v1 ;
					k1 = k + vinegar - v1;
					for (i = 0; i < oil; i++)
					{
						c0 = 0;
						for (j = 0; j < vinegar; j++)
							c0 += GFpow(sk[ij++]) * x[j]; //a[k1][j*oil+i]*x[j]; //
						A[k * (oil + 1) + i] = c0;
					}
					// constant term caused by vinegar, subtract from y1[k]
					c0 = GFpow(sk[ij++]); //   e[k1] ;
